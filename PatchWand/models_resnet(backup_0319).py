...
#         sys.exit()
        if pooling_type == 'max_pooling':
            self.pooling = MyMaxPool1dPadSame(kernel_size=self.stride)
...
#         self.n_classes = training_params['n_classes'][0]
        ...
        self.base_filters = training_params['base_filters']
...
#             [[input_name, ResNet1D(training_params=training_params, input_channel=1)] for input_name in self.input_names]
#         )
...
#             [[task, RespiratoryRegression(num_classes=self.class_N, input_dim=feature_out_dim+self.N_features )] for task in self.tasks]
#         )
...
# #         self.feature_extractors = nn.ModuleDict(
# #             [[input_name, FeatureExtractor_CNN(training_params=training_params, input_channel=1)] for input_name in self.input_names]
# #         )
...
# # #         self.regressors = nn.ModuleDict(
# # #             [[task, RespiratoryRegression(num_classes=class_N, input_dim=feature_out_dim+self.N_features )] for task in self.tasks]
# # #         )
