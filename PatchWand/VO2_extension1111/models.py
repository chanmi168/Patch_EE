...
from VO2_extension1111.models_CNNlight import *
...
class cardioresp_multiverse(nn.Module):
    def __init__(self, class_N=1, training_params=None):
        ...
        self.use_spectral_smoother = training_params['use_spectral_smoother']
        

        if self.use_spectral_smoother:
            self.spectral_smoother = nn.ModuleDict()
            self.spectral_smoother['fft_HR'] = SpectralExtractor(input_channel=training_params['channel_n']*2, n_block=5)
            self.spectral_smoother['fft_RR'] = SpectralExtractor(input_channel=training_params['channel_n']*2, n_block=4)
        ...
        self.regressors[self.main_task] = CardioRespRegression(training_params=training_params, num_classes=self.n_classes, input_dim=self.extracted_dim + training_params['n_demographic'])
    ...
    def forward(self, ecg, scg, ppg, feature):
        ...
        if self.use_channel_atn:
            output['merged-HR_patch'] = self.auxillary_regressors['merged-HR_patch'](merged_HR_deep_features, channel_attention=channel_attention_HR)
        ...
        if self.use_channel_atn:
            output['merged-RR_cosmed'] = self.auxillary_regressors['merged-RR_cosmed'](merged_RR_deep_features, channel_attention=channel_attention_RR)
        ...
        if self.use_spectral_smoother:
            features_fft_HR = self.spectral_smoother['fft_HR'](features_fft_HR)
            features_fft_RR = self.spectral_smoother['fft_RR'](features_fft_RR)
...
class cardioresp_shittyverse(nn.Module):
    def __init__(self, class_N=1, training_params=None):
        super(cardioresp_shittyverse, self).__init__()
        ...
        input_HR_dim = self.feature_extractors['ECG-HR_patch'].last_layer_dim + self.feature_extractors['SCG-HR_patch'].last_layer_dim
        input_HR_dim = input_HR_dim * channel_n
        self.auxillary_regressors['merged-HR_patch'] = vanilla_regression(training_params, num_classes=1, input_dim=input_HR_dim, hidden_dim=training_params['hidden_dim'], n_layers=3)
        
        input_RR_dim = self.feature_extractors['PPG-RR_cosmed'].last_layer_dim + self.feature_extractors['SCG-RR_cosmed'].last_layer_dim
        input_RR_dim = input_RR_dim * channel_n
        self.auxillary_regressors['merged-RR_cosmed'] = vanilla_regression(training_params, num_classes=1, input_dim=input_RR_dim, hidden_dim=training_params['hidden_dim'], n_layers=3)
        ...
        if ('HR_patch' in self.auxillary_tasks) & ('RR_cosmed' in self.auxillary_tasks):
            self.extracted_dim = input_HR_dim + input_RR_dim + 2
        elif ('HR_patch' in self.auxillary_tasks) & ('RR_cosmed' not in self.auxillary_tasks):
            self.extracted_dim = input_HR_dim+1
        elif ('HR_patch' not in self.auxillary_tasks) & ('RR_cosmed' in self.auxillary_tasks):
            self.extracted_dim = input_RR_dim+1
        else:
            self.extracted_dim = 0
            self.empty_tensor = torch.empty(0).float().to(training_params['device'])
    ...
    def forward(self, ecg, scg, ppg, feature):
        ...
        # concat multi-modal ECG and SCG data (time domain)
        merged_HR_deep_features = torch.cat([deep_feature['ECG-HR_patch'], deep_feature['SCG-HR_patch']], axis=1)
        merged_HR_deep_features = merged_HR_deep_features.reshape(merged_HR_deep_features.size()[0],-1)
        output['merged-HR_patch'] = self.auxillary_regressors['merged-HR_patch'](merged_HR_deep_features)

        # concat multi-modal PPG and SCG data (time domain)
        merged_RR_deep_features = torch.cat([deep_feature['PPG-RR_cosmed'], deep_feature['SCG-RR_cosmed']], axis=1)
        merged_RR_deep_features = merged_RR_deep_features.reshape(merged_RR_deep_features.size()[0],-1)
        output['merged-RR_cosmed'] = self.auxillary_regressors['merged-RR_cosmed'](merged_RR_deep_features)
        

        # combine deep features with vital signs
        if ('HR_patch' in self.auxillary_tasks) & ('RR_cosmed' in self.auxillary_tasks):
            merged_deep_features = torch.cat([merged_HR_deep_features, merged_RR_deep_features, output['merged-HR_patch'], output['merged-RR_cosmed']], axis=-1)

        elif ('HR_patch' in self.auxillary_tasks) & ('RR_cosmed' not in self.auxillary_tasks):
            merged_deep_features = torch.cat([merged_HR_deep_features, output['merged-HR_patch']], axis=-1)
            del output['merged-RR_cosmed']
        
        elif ('HR_patch' not in self.auxillary_tasks) & ('RR_cosmed' in self.auxillary_tasks):
            merged_deep_features = torch.cat([merged_RR_deep_features, output['merged-RR_cosmed']], axis=-1)
            del output['merged-HR_patch']
        ...
        # print(merged_deep_features.size())
        # print(self.regressors[self.main_task])
        
        # regression for target variable
        output[self.main_task], concat_feature = self.regressors[self.main_task](merged_deep_features, feature)

        attention_dict = {
            'spectral_attention_HR': np.empty(ecg.size(0)),
            'spectral_attention_RR': np.empty(ecg.size(0)),
            'channel_attention_HR': np.empty(ecg.size(0)),
            'channel_attention_RR': np.empty(ecg.size(0)),
            'features_fft_HR_raw': np.empty(ecg.size(0)),
            'features_fft_RR_raw': np.empty(ecg.size(0)),
        }
        return output, deep_feature, concat_feature, attention_dict
...
class CardioRespRegression(nn.Module):    
    ...
    def __init__(self, training_params, num_classes=10, input_dim=10):
        ...
        self.n_demographic = training_params['n_demographic']

    def forward(self, merged_deep_features, feature):
        concat_feature = torch.cat([merged_deep_features, feature[:,:self.n_demographic]], axis=-1)
...
class DominantFreqRegression(nn.Module):
    ...
    def forward(self, x, spectral_attention=None, channel_attention=None):
        ...
        if spectral_attention is not None:
            out, _ = self.apply_attention(out, spectral_attention, axis=-1)

        if channel_attention is not None:
            _, out = self.apply_attention(out, channel_attention, axis=1)
...
class MultiTaskLoss(nn.Module):
    def __init__(self, training_params):
        ...
        main_loss_weight = 1 - (len(self.regression_names) - 1) * training_params['auxillary_loss_weight']
        ...
        for task_name in self.regression_names:
            if self.main_task in task_name:
                ...
                self.loss_weights[task_name] = main_loss_weight
            else:
    ...
    def forward(self, out, label):
        ...
        for out_name in self.criterions.keys():
            if self.loss_weights[ out_name ]==0:
                losses[out_name] = torch.zeros(1).to(device=self.device, dtype=torch.float).squeeze()
                # losses[out_name] = torch.zeros(out[out_name].squeeze().size()).to(device=self.device, dtype=torch.float).squeeze()
...
#             print(out_name, out[out_name].size())
