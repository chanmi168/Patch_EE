...
class InceptionBlock(nn.Module):
    ...
    def __init__(self,training_params, stride=2, input_channel=None, output_channel=None):
        """
        Inputs:
        ...
        self.ch_pooling = torch.nn.Conv1d(channel_n*3, output_channel, 1) # out_channels->1 channel, kernel size = 1
...
#         self.layer1 =  InceptionBlock(training_params, stride=stride, input_channel=input_channel)
        ...
        self.ch_pooling = torch.nn.Conv1d(channel_n, output_channel, 1) # out_channels->1 channel, kernel size = 1
...
class SpectralExtractor(nn.Module):
    def __init__(self, input_channel, n_block=None):
        ...
        for i_block in range(self.n_block):
            net = nn.Sequential(
                nn.Conv1d(input_channel, input_channel, kernel_size=kernel_size, stride=1, padding=math.floor(kernel_size/2), groups=input_channel),
                nn.BatchNorm1d(input_channel),
                nn.ReLU(),
                nn.Conv1d(input_channel, input_channel, kernel_size=kernel_size, stride=1, padding=math.floor(kernel_size/2), groups=input_channel),
                nn.BatchNorm1d(input_channel),
                nn.ReLU(),
                MyAvgPool1dPadSame(kernel_size=kernel_size, stride=stride),
            )
            self.basicblock_list.append(net)
            


        # self.basicblock_list.append(nn.AdaptiveMaxPool1d(1))
