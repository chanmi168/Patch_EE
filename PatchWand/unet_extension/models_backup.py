...
#                 SELayer(in_ch) if (self.SE_block) or (in_ch>=2) else nn.Identity(),
...
class U_Net(nn.Module):
    ...
    """
    def __init__(self, in_ch=3, out_ch=2, n1=8, training_params=None):
...
#         self.Maxpool4 = nn.MaxPool2d(kernel_size=2, stride=2)
        ...
        self.Conv0 = conv_block(1, 1, training_params=training_params, hidden_ch=filters[0])
#         self.Conv1 = conv_block(filters[0], filters[0], training_params)
        self.Conv1 = conv_block(1, filters[0], training_params=training_params)

#         self.Conv1 = conv_block(in_ch, filters[0], training_params, groups=in_ch)
...
#         self.Up_conv4 = conv_block(filters[3], filters[2], training_params)
        ...
        if 'regressor' in training_params:
...
#             nn.AvgPool2d(kernel_size=2, stride=2),
            ...
            nn.MaxPool2d(kernel_size=2, stride=2),
#             nn.AvgPool2d(kernel_size=2, stride=2),
#             nn.AvgPool2d(kernel_size=3, stride=(4,8)),
#             nn.Conv2d(1, 1, kernel_size=3, stride=1, padding=0,  bias=True),
#             nn.ReLU(inplace=True),
#             nn.AvgPool2d(kernel_size=3, stride=(2,4)),
        )
...
#         y = self.avg_pool(x).view(b, c)/256
#         y = self.fc(y).view(b, c, 1, 1)
#         print(y)
#         print(residual.size())
