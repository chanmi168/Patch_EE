...
#         self.auxillary_regressors['SCG-RR_cosmed'] = DominantFreqRegression(training_params, training_params['RR_xf_dict'])
        ...
        # self.fc = SingleLayer_regression(input_dim=concat_dim, output_dim=self.hidden_dim)
        # self.fc = SingleLayer_regression(input_dim=concat_dim, output_dim=self.hidden_dim)
        # self.fc = SingleLayer_regression(input_dim=concat_dim, output_dim=self.hidden_dim)
        # self.fc = SingleLayer_regression(input_dim=concat_dim, output_dim=self.hidden_dim)
        
        self.regressors[self.main_task] = CardioRespRegression(training_params=training_params, num_classes=self.n_classes, input_dim=self.hidden_dim*4+2+3)
    ...
    def forward(self, ecg, scg, ppg, feature):
        ...
        # spectral_attention_HR.expand(list(features_fft_HR.size())) * features_fft_HR


        # print('channel_attention_HR, channel_attention_RR', channel_attention_HR, channel_attention_RR)
...
#         # merged_RR_deep_features dim: (B,2C)
        ...
        # merged_deep_features = self.fc(merged_deep_features)
        # sys.exit()
