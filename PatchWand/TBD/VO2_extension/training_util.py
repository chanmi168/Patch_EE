...
#     label_arr = np.empty(0)
    ...
    out_dict = {}
    for model_out_name in training_params['regression_names']:
...
#         label = label[:, [training_params['tasks'].index('EE_cosmed')]].data.detach().cpu().numpy()
        ...
        for output_name in out.keys():
            ...
            label_dict[output_name].append(label[:,  [training_params['regression_names'].index(output_name)] ].detach().cpu().numpy())
...
#             total_losses_train[loss_name][epoch] = performance_dict_train['total_losses'][loss_name]
    ...
    if training_params['regressor_type']=='CardioRespXGBRegression':
        ...
        # training_params['xgb_regressor'] = training_params['xgb_regressor'].fit(performance_dict_train['feature_arr'], label_train)        
        # performance_dict_train['out_dict'][main_task] = training_params['xgb_regressor'].predict(performance_dict_train['feature_arr'])
        # performance_dict_val['out_dict'][main_task] = training_params['xgb_regressor'].predict(performance_dict_val['feature_arr'])
        # sys.exit()
