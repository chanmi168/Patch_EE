...
aaa = torch.rand(10,23)
# bbb = torch.rand(10,5)

# torch.cat([aaa, bbb[:,:0]], axis=-1).size()
torch.zeros(aaa.size()).size()
...
#                           '--output_folder', '../../../data/stage4/DFR_DL1111/TEST/',
...
for training_params in [training_params_list[0]]:
    ...
    if training_params['model_arch'] == 'cardioresp_multiverse':
        model_arch = cardioresp_multiverse
    if training_params['model_arch'] == 'cardioresp_shittyverse':
        model_arch = cardioresp_shittyverse
...
# aaa = torch.rand(10, training_params['channel_n']*2, 55)

# net = SpectralExtractor(input_channel=training_params['channel_n']*2, n_block=4)
# net(aaa).shape, net.pytorch_total_params


# In[15]:


# aaa = torch.rand(10, training_params['channel_n']*2, 150)

# net = SpectralExtractor(input_channel=training_params['channel_n']*2, n_block=5)
# net(aaa).shape, net.pytorch_total_params
...
#     def __init__(self, training_params=None, input_channel=None, output_channel=None, n_block=None, xf_dict=None):
...
def ma_test(training_params):
    ...
    # prepare model
    model = model_arch(training_params=training_params)
...
def test_model(training_params):
    ...
    model = model_arch(training_params=training_params_copy)
...
def get_sweep_folder(training_params):
    ...
    n_demographic = training_params['n_demographic']
    ...
    sweep_folder = 'rep{}/auxweight{}-{}-{}feat-{}-{}-{}-{}-{}dem'.format(i_rep, auxillary_weight, adaptive_loss_name, n_feat, auxillary_tasks, training_params['model_arch'], training_params['regressor_type'], use_atten, n_demographic)
        ...
        if training_params['show_plot']:
            ...
            if training_params['model_arch'] == 'cardioresp_multiverse':
                ...
                if 'CNNlight' in training_params['model_name']:
                    ...
                    if ('HR' not in task) & ('RR' not in task):
...
# MTL =  MultiTaskLoss(training_params)
# MTL
